...
def extract_time_series(wr: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if has_operation_flag:
        ...
        if "wind_turbine" not in operating.dims:
            operating_array = np.stack(
                [operating_array] * len(np.asarray(wr_flat["wind_turbine"])),
                axis=1
            )
...
def extract_probability_distribution(wr: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "wind_direction" not in wr['coord']:
        raise ValueError(
            "Probability distribution wind resource requires 'wind_direction' coordinate."
        )
    ...
    if "probability" not in wr['data']:
        raise ValueError(
            "Probability distribution wind resource requires 'probability' data field."
        )
    ...
    ALLOWED_DIMS = (
        ...
        (
            "turbulence_intensity",
            (len(turbulence_intensities),) if turbulence_intensities is has_ti_dim else ()
        ),
    )
    ...
    if "operating" in wr_flat:
        ALLOWED_DIMS = (
            (
                "wind_turbine",
                (len(np.asarray(wr_flat["wind_turbine"])),) if "wind_turbine" in wr_flat else ()
            ),
        )
...
def weibull_distribution_to_probability_distribution(wr: dict) -> dict:
    ...
    ALLOWED_DIMS = (
        ("wind_direction", wr_flat.get("wind_direction", np.array([])).shape),
