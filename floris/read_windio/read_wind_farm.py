...
def create_generic_power_curve(
    ...
    for i, ws in enumerate(power_wind_speeds):
        if ws < cutin_wind_speed:
            ...
            power_data[i] = (
                rated_power
                * ((ws - cutin_wind_speed) / (rated_wind_speed - cutin_wind_speed))**2
            )
...
def read_single_turbine_type(turbine_windio: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ("Cp_curve" in performance_windio.data):
        ...
        mult_factor = (
            0.5
            * 1.225  # TODO: Is air density always 1.225 if WindIO Cp specification?
            * (np.pi * (turbine_floris["rotor_diameter"]/2)**2 * wind_speed**3)
        )
...
def read_wind_farm(windio_dict: Dict[str, Any], logger) -> Dict[str, Any]:
    ...
    with TrackedDict(windio_dict) as wind_farm_windio:
        ...
        if (isinstance(layouts_windio, list)):
            ...
            layout_windio = TrackedDict.from_list(
                layouts_windio, context=wind_farm_windio.context + ".layouts"
            )[0]
        else:
        ...
        if ('turbines' in wind_farm_windio.data):
            if ("turbine_types" in wind_farm_windio.data):
                raise ValueError(
                    "Both turbines and turbine_types defined in wind_farm, inconsistent data"
                )

            if (turbine_type_is_unique):
                raise ValueError(
                    "Turbines defined in wind_farm but multiple turbine types found, inconsistent "
                    "data"
                )
        ...
        # Multi-turbine type case
            ...
            if isinstance(turbine_types, list):
                turbine_types = dict(enumerate(turbine_types))
    ...
    # -- Process individual turbine types ------------------------------------ #
        ...
        if (layout_z is not None):
            print(
                "WARNING: Direct altitude is not supported in FLORIS, adjusting hub heights "
                "accordingly."
            )
            for idx in range(len(farm_floris["layout_x"])):
                farm_floris["turbine_type"][idx]["hub_height"] = (
                    layout_z[idx] + farm_floris["turbine_type"][idx]["hub_height"]
                )
